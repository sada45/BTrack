...
class fingerprint_group_dataset(Dataset):
    def __init__(self, f_name,  in_chan, group_size, include=None, exclude=None):
        ...
        if torch.max(self.label[:, -1]) > 1:
            raise Exception("exclude has larger than 1")
        # for i in range(self.origin_label.shape[1]):
        #     ee = np.where(self.origin_label[:, i]==1)[0]
        #     print(i, len(ee))
...
def model_eval(classifier, test_dataset):
    ...
    classifier.eval()
    with torch.no_grad():
        for idx, (data, label, _) in enumerate(test_dataloader):
            data = data.to(device)
    ...
    with torch.no_grad():
        for idx, (data, label, origin_label) in enumerate(test_dataloader):
            ...
            for target_id in alien_ids:
                ...
                # if a_idx.shape[0] > 0 and correct_alien_num < 10:
                #     print(predict_idx[a_idx][:10], origin_label[a_idx][:10])
...
def model_train_main(classifier, train_dataset, group_size, include, aliens, in_chan, epoch=20, step_num=-1, confi_lim=0.6, exclude=None):
    if exclude is None:
        exclude = []
    if len(train_dataset) % 64 < 16:
        ...
        for idx, (data, label, _) in enumerate(train_dataloader):
            ...
            if step % 50 == 0:
                ...
                for dis in [1, 3, 5, 7, 9, 11]:
                    if dis == 1:
                        test_data = fingerprint_others_dataset(dataset_path.format(dis), in_chan=in_chan, include=include, exclude=exclude+aliens, train=False)
...
def model_train_others(f_name, in_chan, include, exclude, alien, group_size, feat_len, epoch=20, step_num=-1, confi_lim=0.6):
    ...
    return model_train_main(classifier, train_dataset, group_size, include, alien, in_chan, epoch, step_num, confi_lim, exclude=exclude)
...
##############################
"""
The "others" set has two requirements: 1. general enough, 2. do not similar with the target set
"""
def nrf_main(load=False, group_size=3):
    # near dc = 8e5
    global dev_name, dataset_path
    dataset_path = "./data/nn_data/nrf_{}m.npz"
    dev_name = "nrf"
    ...
    confi_lim = 0.8
    print(include, exlcude, alien)
    if load:
        # params = torch.load("./models/goods/nrf/nrf_classifer_550.pt")
        params = torch.load("./models/goods/nrf/nrf_classifer_550.pt")
        ...
        classifier = model_train_others(dataset_path.format(1), chan, include, exlcude, alien, group_size=group_size, feat_len=feat_lens[dev_name], epoch=50, confi_lim=confi_lim)
...
def cc2650_main(load=False, group_size=3):
    global dev_name, dataset_path
    dataset_path = "./data/nn_data/cc2650_{}m.npz"
    dev_name = "cc2650"
    ...
    confi_lim = 0.8
    if load:
        params = torch.load("./models/goods/cc2650/cc2650_classifer_350.pt")
        classifier = classification_model.FeatureExtractor(2, len(chan), feat_lens[dev_name]).to(device)
        classifier.load_state_dict(params)
    else:
        classifier = model_train_others(dataset_path.format(1), chan, include, exlcude, alien, group_size=group_size, feat_len=feat_lens[dev_name], epoch=60, confi_lim=confi_lim)
        torch.save(classifier.state_dict(), "./models/cc2650_classifer_final.pt")
...
def da_main(load=False, group_size=1):
    global dev_name, dataset_path
    dataset_path = "./data/nn_data/da_{}m.npz"
    dev_name = "da"
    ...
    confi_lim = 0.8
    if load:
        params = torch.load("./models/goods/da/da_classifer_750.pt")
        classifier = classification_model.FeatureExtractor(len(include)+1, len(chan), feat_len=feat_lens[dev_name]).to(device)
        classifier.load_state_dict(params)
    else:
        classifier = model_train_others(dataset_path.format(1), chan, include, exlcude, alien, group_size=group_size, feat_len=feat_lens[dev_name], epoch=50, confi_lim=confi_lim)
    ...
    for dis in [1, 3, 5, 7, 9, 11]:
        if dis == 1:
            test_data = fingerprint_others_dataset(dataset_path.format(dis), in_chan=chan, include=include, train=False, exclude=exlcude+alien)
        else:
            test_data = fingerprint_group_dataset(dataset_path.format(dis), in_chan=chan, group_size=group_size, include=include, exclude=exlcude+alien)
        acc = model_others_eval(classifier, test_data, alien_ids=alien, group_size=group_size, confi_lim=confi_lim)
        print(dis, acc)
...
"""
The office sceinaro we used for distance evaluation has lots of interference, 
so we see "sudden drops" of the target identification accuary, especially between 3m~7m.
We just repeat data collection to alleviate the impact and the result still show accuracy drop between 3m~7m
