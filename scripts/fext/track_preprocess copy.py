...
path = "/data/blueprint/raw_data/track/{}/d={}m_ttt_{}_nodeid_{}_chan=37.npz"
    ...
    for key in ["nrf"]:
        ...
        for dis in [1, 5, 10, 11, 15]:
            if dis == 1:
                ...
                f = bfe.get_features(path.format(key, dis, "{}", "{}"), [i for i in range(0, 4)], test_id[key], -1, native=is_natives[key], bias=0, origin_id=node_id[key], pre_cfo=pre_cfos[key], feat_len=feat_lens[key])
            ...
            np.savez("./data/nn_data/track_{}_{}.npz".format(key, dis), data=ramp_seg[valid_idx, :], label=labels[valid_idx, :], cfo=f[0][valid_idx], pdr=f[11])
