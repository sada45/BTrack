...
path = "/data/blueprint/raw_data/unique/{}/d=0m_ttt_{}_nodeid_{}_chan=37.npz"
...
if __name__ == "__main__":
    for key in ["nrf"]:
        ...
        # 1, 3, 5, 7, 9, 11
        f = bfe.get_features(path.format(key, "{}", "{}"), [i for i in range(0, 4)], node_id[key], -1, native=is_natives[key], bias=0, pre_cfo=None, feat_len=feat_lens[key])
        cluster_info, _ = cluster_cfo(f[0])
        print(cluster_info)
        st_info = np.array([np.mean(f[1]), np.std(f[1])])
        ...
        np.savez("./data/nn_data/unique_{}.npz".format(key), data=ramp_seg[valid_idx, :], label=labels[valid_idx, :], cfo=f[0][valid_idx])
