        ...
        for idx, (data, label, _) in enumerate(train_dataloader):
            ...
            if step % 50 == 0:
                ...
                for dis in [1]:
                    ...
                    print(dis, acc, test_data.pdr)
                    torch.save(classifier.state_dict(), "./models/track_{}_classifer_{}.pt".format(dev_name, step))
                classifier.train()
...
##############################
"""
The "others" set has two requirements: 1. general enough, 2. do not similar as the target set
"""
def nrf_main(load=False, group_size=3):
    # near dc = 8e5
    global dev_name, dataset_path
    dataset_path = "./data/nn_data/track_nrf_{}.npz"
    dev_name = "nrf"
    ...
    confi_lim = 0.8
    if load:
        params = torch.load("./models/track/nrf/track_nrf_classifer_2500.pt")
        ...
        torch.save(classifier.state_dict(), "./models/track_{}_classifer_final.pt".format(dev_name))
    
    for dis in [1, 5, 10, 11, 15]:
        if dis == 1:
...
#     ori_pdr = []
#     for dis in [1, 5, 10, 11, 15]:
#         p = np.load("/data/blueprint/data/nn_data/track_nrf_{}.npz".format(dis))["pdr"]
#         ori_pdr.append(p)
#     pdr = []
#     root = (1 / 40) * 47 * 8
#     for i in ori_pdr:
